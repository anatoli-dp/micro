...
void markObject(GC *gc, Obj *object)
{
    ...
    if (gc->grayCapacity < gc->grayCount + 1)
    {
        ...
        gc->grayStack = (Obj **) realloc(gc->grayStack, sizeof(Obj *) * gc->grayCapacity);
...
{
    for (int i = 0; i < array->count; i++) markValue(gc, array->values[i]);
}
...
{
    if (debugLogGC) printf("%p free type %d\n", (void *) object, object->type);
...
void collectGarbage(GC *gc)
{
    ...
    if (gc->markRoots) gc->markRoots(gc, gc->markRootsArg);
    traceReferences(gc);
    if (gc->fixWeak) gc->fixWeak(gc->fixWeakArg);
    sweep(gc);

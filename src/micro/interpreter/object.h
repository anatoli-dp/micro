...
typedef struct
{
    ...
    Chunk chunk;
    ObjString *name;
    // microasm scratch requirements (compiled maximums per function)
    uint16_t micro_i_slots; // number of 8‑byte int slots needed (0 if none)
    uint16_t micro_f_slots; // number of 8‑byte float slots needed (0 if none)
} ObjFunction;

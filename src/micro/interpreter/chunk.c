...
{
    FREE_ARRAY(gc, uint16_t, chunk->code, chunk->capacity);
...
void writeChunk(GC *gc, Chunk *chunk, uint16_t byte, int line)
{
    if (chunk->capacity < chunk->count + 1)
    {
        ...
        chunk->code = GROW_ARRAY(gc, uint16_t, chunk->code, oldCapacity, chunk->capacity);
        chunk->lines = GROW_ARRAY(gc, int, chunk->lines, oldCapacity, chunk->capacity);
    }
